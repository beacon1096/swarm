...
- Private services use the in-cluster `tailscale` operator to expose individual ClusterIPs as Tailscale-only endpoints.
- No NodePort exposure to the LAN by default.
- Cross-cluster reachability between talos-i and talos-ii also goes via Tailscale (PodCIDRs and ServiceCIDRs are non-overlapping by design but routing them at L3 isn't currently desired — Tailscale gives per-service control).
...
**Version:** 1.1.0
**Last ratified:** 2026-04-27 — split principles by cluster scope, add Cloudflare-vs-VPS exposure rule, separate self-hosted factory permission for talos-i.
