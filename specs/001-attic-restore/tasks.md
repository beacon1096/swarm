...
- [ ] T017 [US1] Watch flux reconcile, then verify the public route + tailnet device are alive: `kubectl get httproute -n nix attic -o yaml | yq '.status'` (expect `Accepted=True` from `envoy-external`); `kubectl get pods -n network | grep ts-attic` (expect a `ts-attic-<id>-0` Pod `Running` — the tailscale-operator targets `network` namespace, so its per-Service ts-proxy Pods land in `network`, **not** in a separate `tailscale` namespace); `kubectl logs -n network ts-attic-<id>-0 | tail -20` (expect successful tailnet login, **not** `NeedsLogin`).
- [ ] T018 [US1] Create the `nix-fleet` cache. **`atticadm` in this version has no `create-cache` / `list-caches` subcommands** — go through the REST API instead. Mint a short-lived (5 m) admin-scope token first (`atticadm` has no admin/op distinction in this version: a token with `--pull` + `--push` on the target cache name reaches cache-config endpoints; the 5 m validity is the safety net), then `POST /_api/v1/cache-config/nix-fleet` from inside the cluster. The Host header MUST be one of the entries in `server.toml`'s `allowed-hosts` (use `Host: nix.beaco.works`). Body:

  ```json
  {"keypair":"Generate","is_public":false,"store_dir":"/nix/store","priority":41,"upstream_cache_key_names":[],"retention_period":null}
  ```

  Verify with `GET /_api/v1/cache-config/nix-fleet` returning HTTP 200 + a JSON body containing `public_key`. Full curl/kubectl invocation is in `~/swarm/docs/operations/attic-restore.md` §"Adding a new cache".
- [ ] T019 [US1] Mint the long-lived pull token: `kubectl exec -n nix deploy/attic -- atticadm -f /config/server.toml make-token --sub nix-daemon --validity 1y --pull nix-fleet`. Capture the JWT. **Do not** paste it into any log file in this repo. Note: the short-lived admin-scope token used in T018 is minted the same way (`atticadm make-token`), differing only by `--validity 5m` and by adding `--push nix-fleet`. There is **no** separate `admin` scope in this version of atticadm — what gives the token enough privilege to call cache-config endpoints is having both `--pull` and `--push` on the target cache; document this as a known limitation in the runbook (a real admin scope may land in a future attic release).
...
- [ ] T031 [US3] Revoke the old (pre-T028) token. **`atticadm revoke-token` does not exist in this version of the binary** — there is no direct revocation operation. Two compensating patterns: (a) **short-validity rotation** — accept that the old token is dead at its natural expiry, and for emergencies mint a `--validity 1d` replacement and let the leaked one age out within 24 h; (b) **RS256 keypair rotation** — replace `attic-secret.token-rs256-secret-base64` and `…public-key-base64` (regenerate per `contracts/attic-server-toml.md`), commit, flux + pod restart. This invalidates **all** outstanding tokens at once (every existing JWT fails verification against the new public key); coordinate with `nix-daemon`s + CI + every other token holder before pulling the lever. SC-006 acceptance ("old token → 401") in this version is satisfied either by waiting out validity, or — for immediate proof — by performing the keypair rotation and re-curling with the old JWT (expect `HTTP/2 401`). Both procedures live in `~/swarm/docs/operations/attic-restore.md` §"Token revocation".
