...
- [X] T002 Verify Longhorn r2 storage class exists and has node capacity: `kubectl get sc longhorn-r2` (must show `Provisioner: driver.longhorn.io`); `kubectl -n longhorn-system get nodes.longhorn.io -o wide` (each node `Schedulable: true` with ≥ 600 Gi free in the `longhorn-r2` disk tag).
...
- [X] T014 [P] Create PVC at `/home/beacon/swarm/kubernetes/apps/registry/zot/app/pvc.yaml`: name `zot-store`, namespace `registry`, `accessModes: [ReadWriteOnce]`, `storageClassName: longhorn-r2`, `resources.requests.storage: 300Gi`. Pattern: clone `/home/beacon/swarm/kubernetes/apps/nix/attic/app/pvc.yaml`. **DEVIATION**: chart 0.1.79 `templates/statefulset.yaml` always builds a `volumeClaimTemplates` from `pvc.*` values when `persistence: true` and does NOT honor `existingClaim`. Pre-creating a separate PVC would dangle. Pivoted to chart-managed VCT (`pvc.storage: 300Gi`, `pvc.storageClassName: longhorn-r2`); pvc.yaml NOT created. PVC will materialize at runtime as `zot-pvc-zot-0`.
