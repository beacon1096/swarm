...
- `act_runner`: `code.forgejo.org/forgejo/runner:12.7.3-amd64` (matches swarm-01; Plan-stage check at apply time for newer 12.7.x patches). Pulled through LAN zot mirror (zot proxies `code.forgejo.org`).
- `dind`: `172.16.80.240:5000/library/docker:29.2.1-dind-amd64` in Phase 1 (LAN zot pull-through cache; matches swarm-01 pin). Plan-stage 5-minute spot-check on Docker Hub `library/docker` tag list — bump only if `29.2.x` security patch released.
...
- **Wrong namespace label level** — PSA labels MUST be `privileged`, not `baseline` or `restricted`. The dind container's `privileged: true` is rejected with cryptic admission error if any of the three labels is missing or stricter. Verifiable post-apply: `kubectl get ns forgejo-runner -o jsonpath='{.metadata.labels}'` shows all three keys at `privileged`.
